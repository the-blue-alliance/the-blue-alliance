...
class TestEventGetTimezoneId(unittest2.TestCase):
    ...
    def test_2012ct_no_location(self):
        ...
        event['location'] = None
        self.assertEqual(EventHelper.get_lat_lon(event['location']), None)
        self.assertEqual(EventHelper.get_timezone_id(event['location']), None)
    ...
    def test_2012ct(self):
        ...
        self.assertEqual(EventHelper.get_lat_lon(event['location']), (41.76371109999999, -72.6850932))
        self.assertEqual(EventHelper.get_timezone_id(event['location']), 'America/New_York')

    def test_2013flbr(self):
        with open('test_data/usfirst_html/usfirst_event_details_2013flbr.html', 'r') as f:
            event, _ = UsfirstEventDetailsParser.parse(f.read())

        self.assertEqual(EventHelper.get_lat_lon(event['location']), (26.1224386, -80.13731740000001))
        self.assertEqual(EventHelper.get_timezone_id(event['location']), 'America/New_York')
    ...
    def test_2013casj(self):
        ...
        self.assertEqual(EventHelper.get_lat_lon(event['location']), (37.3382082, -121.8863286))
        self.assertEqual(EventHelper.get_timezone_id(event['location']), 'America/Los_Angeles')
    ...
    def test_2005is(self):
        ...
        self.assertEqual(EventHelper.get_lat_lon(event['location']), (32.7940463, 34.989571))
        self.assertEqual(EventHelper.get_timezone_id(event['location']), 'Asia/Jerusalem')

    def test_2005or(self):
        with open('test_data/usfirst_html/usfirst_event_details_2005or.html', 'r') as f:
            event, _ = UsfirstEventDetailsParser.parse(f.read())

        self.assertEqual(EventHelper.get_lat_lon(event['location']), (45.5230622, -122.6764816))
        self.assertEqual(EventHelper.get_timezone_id(event['location']), 'America/Los_Angeles')
    ...
    def test_1997il(self):
        ...
        self.assertEqual(EventHelper.get_lat_lon(event['location']), (41.8781136, -87.6297982))
        self.assertEqual(EventHelper.get_timezone_id(event['location']), 'America/Chicago')
