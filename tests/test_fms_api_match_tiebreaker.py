...
class TestFMSAPIMatchTiebreaker(unittest2.TestCase):
    ...
    def test_2017flwp_sequence(self):
        ...
        for item in json.loads(result.content)['items']:
            ...
            MatchManipulator.createOrUpdate(
                DatafeedFMSAPI('v2.0', sim_time=query_time).getMatches(
                    '2017{}'.format(event_code)),
    ...
    def test_2017flwp(self):
        ...
        MatchManipulator.createOrUpdate(
            DatafeedFMSAPI(
                'v2.0', sim_time=datetime.datetime(2017, 3, 04, 21,
                                                   22)).getMatches('2017flwp'))
        MatchHelper.deleteInvalidMatches(event.matches, event)
    ...
    def test_2017pahat(self):
        ...
        MatchManipulator.createOrUpdate(
            DatafeedFMSAPI(
                'v2.0', sim_time=datetime.datetime(
                    2017, 3, 05, 20, 45)).getMatches('2017pahat'))
        MatchHelper.deleteInvalidMatches(event.matches, event)
        f_matches = Match.query(Match.event == ndb.Key(Event, '2017pahat'),
        ...
        MatchManipulator.createOrUpdate(
            DatafeedFMSAPI(
                'v2.0', sim_time=datetime.datetime(
                    2017, 3, 05, 21, 02)).getMatches('2017pahat'))
        MatchHelper.deleteInvalidMatches(event.matches, event)
        f_matches = Match.query(Match.event == ndb.Key(Event, '2017pahat'),
                                Match.comp_level == 'f').fetch()
        ...
        self.assertEqual(
            tiebreaker_match.score_breakdown['red']['totalPoints'], 240)
        self.assertEqual(
            tiebreaker_match.score_breakdown['blue']['totalPoints'], 235)
    ...
    def test_2017scmb_sequence(self):
        ...
        sf_matches = Match.query(Match.event == ndb.Key(Event, '2017scmb'),
                                 Match.comp_level == 'sf').fetch()
        self.assertEqual(len(sf_matches), 4)

        f_matches = Match.query(Match.event == ndb.Key(Event, '2017scmb'),
                                Match.comp_level == 'f').fetch()
        self.assertEqual(len(f_matches), 3)
    ...
    def test_2017scmb(self):
        ...
        MatchManipulator.createOrUpdate(
            DatafeedFMSAPI(
                'v2.0', sim_time=datetime.datetime(2017, 3, 04, 19,
                                                   17)).getMatches('2017scmb'))
        MatchHelper.deleteInvalidMatches(event.matches, event)
        qf_matches = Match.query(Match.event == ndb.Key(Event, '2017scmb'),
                                 Match.comp_level == 'qf').fetch()
        ...
        MatchManipulator.createOrUpdate(
            DatafeedFMSAPI(
                'v2.0', sim_time=datetime.datetime(2017, 3, 04, 20,
                                                   12)).getMatches('2017scmb'))
        MatchHelper.deleteInvalidMatches(event.matches, event)
        qf_matches = Match.query(Match.event == ndb.Key(Event, '2017scmb'),
                                 Match.comp_level == 'qf').fetch()
        self.assertEqual(len(qf_matches), 12)
    ...
    def test_2017ncwin(self):
        ...
        MatchManipulator.createOrUpdate(
            DatafeedFMSAPI(
                'v2.0', sim_time=datetime.datetime(2017, 3, 05, 21,
                                                   2)).getMatches('2017ncwin'))
        MatchHelper.deleteInvalidMatches(event.matches, event)
