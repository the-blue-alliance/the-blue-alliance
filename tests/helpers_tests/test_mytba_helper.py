...
class TestMyBAHelper(unittest2.TestCase):
    ...
    def test_add_favorite_200(self):
        ...
        with patch.object(TBANSHelper, 'update_favorites') as mock_update_favorites, \
             patch.object(favorite, 'put') as mock_put:
                self.assertEqual(MyTBAHelper.add_favorite(favorite), 200)
                mock_put.assert_called_once()
                mock_update_favorites.assert_called_once_with('user_id', None)
    ...
    def test_add_favorite_200_sending_device_key(self):
        ...
        with patch.object(TBANSHelper, 'update_favorites') as mock_update_favorites, \
             patch.object(favorite, 'put') as mock_put:
                ...
                mock_put.assert_called_once()
                mock_update_favorites.assert_called_once_with('user_id', 'sending_device_key')
    ...
    def test_add_favorite_304(self):
        ...
        with patch.object(TBANSHelper, 'update_favorites') as mock_update_favorites:
            ...
            mock_update_favorites.assert_not_called()
    ...
    def test_add_subscription_200(self):
        ...
        with patch.object(TBANSHelper, 'update_subscriptions') as mock_update_subscriptions, \
             patch.object(subscription, 'put') as mock_put:
                self.assertEqual(MyTBAHelper.add_subscription(subscription), 200)
                mock_put.assert_called_once()
                mock_update_subscriptions.assert_called_once_with('user_id', None)
    ...
    def test_add_subscription_200_sending_device_key(self):
        ...
        with patch.object(TBANSHelper, 'update_subscriptions') as mock_update_subscriptions, \
             patch.object(subscription, 'put') as mock_put:
                ...
                mock_put.assert_called_once()
                mock_update_subscriptions.assert_called_once_with('user_id', 'sending_device_key')
    ...
    def test_remove_subscription_404(self):
        ...
        with patch.object(TBANSHelper, 'update_subscriptions') as mock_update_subscriptions:
            ...
            mock_update_subscriptions.assert_not_called()
