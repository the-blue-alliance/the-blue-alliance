...
class TestApiController(unittest2.TestCase):
    ...
    def test_validate_tba_app_id(self):
        ...
        response = self.testapp.get(
            '/2010sc', headers={'X-TBA-App-Id': ''}, expect_errors=True)
        self.assertEqual(response.status_code, 400)
        self.assertTrue('Error' in response.json)
        ...
        # Fail
        response = self.testapp.get(
            '/2010sc', headers={'X-TBA-App-Id': 'a::'}, expect_errors=True)
        self.assertEqual(response.status_code, 400)
        self.assertTrue('Error' in response.json)

        # Fail
        response = self.testapp.get(
            '/2010sc', headers={'X-TBA-App-Id': 'a:a:'}, expect_errors=True)
        self.assertEqual(response.status_code, 400)
        self.assertTrue('Error' in response.json)

        # Pass, event not in database
        response = self.testapp.get(
            '/2010sc', headers={'X-TBA-App-Id': 'a:a:a'}, expect_errors=True)
