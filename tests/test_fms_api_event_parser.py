...
class TestFMSAPIEventParser(unittest2.TestCase):
    ...
    def test_parseAwards(self):
        ...
        for award in awards:
            if award.key.id() == '2015waamv_3':
                ...
                self.assertEqual(award.name_str,
    ...
    def test_parseMatches(self):
        with open('test_data/fms_api/2015waamv_staging_matches.json',
                  'r') as f:
            matches, _ = FMSAPIHybridScheduleParser(2015, 'waamv').parse(
                json.loads(f.read()))
        ...
        self.assertEqual(match.team_key_names, [
            u'frc4131', u'frc4469', u'frc3663', u'frc3684', u'frc5295',
            u'frc2976'
        ])
        self.assertEqual(
            match.alliances_json,
            """{"blue": {"surrogates": [], "score": 30, "teams": ["frc4131", "frc4469", "frc3663"]}, "red": {"surrogates": [], "score": 18, "teams": ["frc3684", "frc5295", "frc2976"]}}"""
        )
        self.assertEqual(match.time, datetime.datetime(2015, 2, 27, 0, 0))
        self.assertEqual(match.actual_time,
        ...
        self.assertEqual(match.team_key_names, [
            u'frc3663', u'frc5295', u'frc2907', u'frc2046', u'frc3218',
            u'frc2412'
        ])
        self.assertEqual(
            match.alliances_json,
            """{"blue": {"surrogates": [], "score": null, "teams": ["frc3663", "frc5295", "frc2907"]}, "red": {"surrogates": [], "score": null, "teams": ["frc2046", "frc3218", "frc2412"]}}"""
        )
    ...
    def test_parseEventRankings(self):
        with open('test_data/fms_api/2015waamv_staging_rankings.json',
                  'r') as f:
            rankings = FMSAPIEventRankingsParser(2015).parse(
                json.loads(f.read()))
        ...
        ], [31, 2557, 15, 6, 0, 0, 0, 36, 2], [32, 1318, 1, 6, 0, 0, 0, 8, 2]])
