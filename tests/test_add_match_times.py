...
class TestAddMatchTimes(unittest2.TestCase):
    ...
    def matchDictToMatches(self, match_dicts):
        return [
            Match(
                id=Match.renderKeyName(self.event.key.id(),
    ...
    def test_match_times(self):
        with open('test_data/usfirst_html/usfirst_event_matches_2013cama.html',
                  'r') as f:  # using matches from a random event as data
        ...
        self.assertEqual(
            matches[0].time,
            datetime.datetime(2014, 2, 28, 9, 0) -
            datetime.timedelta(hours=PST_OFFSET))
        self.assertEqual(
            matches[75].time,
            datetime.datetime(2014, 3, 1, 11, 50) -
            datetime.timedelta(hours=PST_OFFSET))
    ...
    def test_match_times_dst(self):
        ...
        self.assertEqual(
            matches[0].time,
            datetime.datetime(2014, 3, 8, 9, 0) -
            datetime.timedelta(hours=PST_OFFSET))
        self.assertEqual(
            matches[-1].time,
            datetime.datetime(2014, 3, 9, 16, 5) -
            datetime.timedelta(hours=PDT_OFFSET))
