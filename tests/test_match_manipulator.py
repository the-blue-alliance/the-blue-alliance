...
class TestMatchManipulator(unittest2.TestCase):
    ...
    def test_createOrUpdate(self):
        ...
        self.assertEqual(
            Match.get_by_id("2012ct_qm1").score_breakdown['red']['auto'], 20)
    ...
    def test_findOrSpawn(self):
        ...
        self.assertMergedMatch(
            MatchManipulator.findOrSpawn(self.new_match), True)
    ...
    def test_updateMerge_no_auto_union(self):
        self.assertMergedMatch(
            MatchManipulator.updateMerge(
                self.new_match, self.old_match, auto_union=False), False)
