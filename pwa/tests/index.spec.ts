...
test('homepage hero heading and tagline are visible', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: 'The Blue Alliance' }),
  ).toBeVisible();
  await expect(
    page.getByText(
...
test('(mobile) homepage hero heading and tagline are visible', async ({
  page,
}) => {
  await expect(
    page.getByRole('heading', { name: 'The Blue Alliance' }),
