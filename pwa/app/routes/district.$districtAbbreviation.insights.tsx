...
export const Route = createFileRoute(
  ...
  loader: async ({ params, context: { queryClient } }) => {
    ...
    // awards are fetched from the browser (see DistrictInsightsPage) rather than
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        {
          name: 'description',
          content: `All-time insights for the ${loaderData.history[loaderData.history.length - 1].display_name} District.`,
        },
      ],
    };
  },
...
function mapToRankings(
  map: Map<string, number>,
): LeaderboardInsight['data']['rankings'] {
  // Group keys by value
    ...
    <div className="py-8">
      ...
      >
        <div className={cn(districtColor && 'border-l-4 pl-4', districtColor)}>
          <h1 className="text-3xl font-medium">{displayName} Insights</h1>
        </div>
        <YearSelector
