...
class DatabaseQuery(object):
    ...
    DATABASE_HITS_MEMCACHE_KEYS = [
        'database_query_hits_{}:{}'.format(i, DATABASE_QUERY_VERSION)
        for i in range(25)
    ]
    DATABASE_MISSES_MEMCACHE_KEYS = [
        'database_query_misses_{}:{}'.format(i, DATABASE_QUERY_VERSION)
        for i in range(25)
    ]
        ...
        for cache_key in cache_keys:
            ...
            if cls.DICT_CONVERTER is not None:
                all_cache_keys += [
                    ...
                    for valid_dict_version in cls.VALID_DICT_VERSIONS
                ]
        logging.info("Deleting db query cache keys: {}".format(all_cache_keys))
        ndb.delete_multi([
            ndb.Key(CachedQueryResult, cache_key)
    ...
    def _dict_cache_key(cls, cache_key, dict_version):
        return '{}~dictv{}.{}'.format(
            cache_key, dict_version,
            cls.DICT_CONVERTER.SUBVERSIONS[dict_version])
