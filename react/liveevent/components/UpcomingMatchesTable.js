    ...
    this.props.matches.forEach((match) => {
      ...
      matchRows.push(
        <tr key={`${match.key}_red`}>
          <td rowSpan="2">
            <a href={`/match/${match.key}`}>
              {getCompLevelStr(match)}
              <br />
              {getMatchSetStr(match)}
            </a>
          </td>
          {match.rt.map((teamKey) => {
            const teamNum = teamKey.substring(3);
            return (
              <td key={teamKey} className="red">
                <a href={`/team/${teamNum}/${this.props.year}`}>{teamNum}</a>
              </td>
            );
          })}
