...
class DatafeedFIRSTElasticSearch(object):
    ...
    def _parse(self, url, parser):
        headers = {}
        try:
            result = urlfetch.fetch(url, headers=headers, deadline=5)
        except Exception, e:
    ...
    def getEventList(self, year):
        events = self._parse(self.EVENT_LIST_URL_PATTERN % (year),
                             FIRSTElasticSearchEventListParser(year))
        return events

    def getEventDetails(self, event):
        if event.first_eid is None:
            logging.warning(
                "Cannot get event details for {}! No first_eid.".format(
                    event.key.id()))
            return None
        return self._parse(self.EVENT_DETAILS_URL_PATTERN % (event.first_eid),
                           FIRSTElasticSearchEventListParser(event.year))[0]

    def getEventTeams(self, event):
        if event.first_eid is None:
            logging.warning(
                "Cannot get event teams for {}! No first_eid.".format(
                    event.key.id()))
            return []

        teams = self._parse(self.EVENT_TEAMS_URL_PATTERN % (event.first_eid,
                                                            event.year),
                            FIRSTElasticSearchTeamDetailsParser(event.year))
        return teams

    def getTeamDetails(self, team):
        if team.first_tpid is None or team.first_tpid_year is None:
            logging.warning(
                "Cannot get team details for {}! No first_tpid or first_tpid_year.".
                format(team.key.id()))
            return None

        return self._parse(self.TEAM_DETAILS_URL_PATTERN % (team.first_tpid),
                           FIRSTElasticSearchTeamDetailsParser(
                               team.first_tpid_year))[0]
