...
class MatchSuggestionHandler(LoggedInHandler):
    def get_qual_bluezone_score(self, prediction):
        return min(100, (
            min(prediction['red']['pressure'] * prediction['red']['pressure'],
                3600) / 1200 + min(prediction['blue']['pressure'] *
                                   prediction['blue']['pressure'], 3600) / 1200
            + min(prediction['red']['gears'] * prediction['red']['gears'], 144
                  ) / 64 + min(prediction['blue']['gears'] * prediction['blue']
                               ['gears'], 144) / 64) * 25)

    def get_elim_bluezone_score(self, prediction):
        return min(100, (
            min(prediction['red']['pressure'] * prediction['red']['pressure'],
                8000) / 3000 + min(prediction['blue']['pressure'] *
                                   prediction['blue']['pressure'], 8000) / 3000
            + min(prediction['red']['gears'] * prediction['red']['gears'], 144
                  ) / 72 + min(prediction['blue']['gears'] * prediction['blue']
                               ['gears'], 144) / 72) * 25)
    ...
    def get(self):
        ...
        for event in current_events:
            ...
            for i, match in enumerate(
                ...
                if not event.details.predictions or match.key.id(
                ) not in event.details.predictions['match_predictions']['qual'
                                                                        if
                                                                        match.
                                                                        comp_level
                                                                        == 'qm'
                    ...
                    match.prediction = event.details.predictions[
                        'match_predictions']['qual' if match.comp_level == 'qm'
                                             else 'playoff'][match.key.id()]
        ...
        finished_matches = sorted(
            finished_matches,
            key=lambda m: m.actual_time if m.actual_time else m.time)
        current_matches = sorted(
            current_matches,
            key=lambda m: m.predicted_time if m.predicted_time else m.time)
