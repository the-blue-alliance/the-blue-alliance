...
class BackupControllerBase(webapp.RequestHandler):
    ...
    def get_backup_bucket(self):
        backup_config_sitevar = self.get_backup_config()
        return backup_config_sitevar.contents.get('datastore_backup_bucket',
                                                  'tba-prod-rawbackups')
    ...
    def check_backup_exists(self, backup_bucket, file_name):
        try:
            ...
            logging.info(
                "Unable to find backup {} in GCS bucket {} - {}".format(
                    file_name, backup_bucket, e.message))
    ...
    def fetch_url(self,
                  ...
                  headers=None):
        try:
            logging.info("Fetching {}, payload={}, headers={}".format(
                url, payload, headers))
...
class DatastoreBackupArchiveFile(BackupControllerBase):
    def get_object_metadata(self, bucket_name, object_name, access_token):
        ...
        status, content = self.fetch_url(url=url, headers=headers)
        return json.loads(content) if status == 200 else {}

    def set_file_storage_class(self, bucket_name, object_name, storage_class,
                               access_token):
        bucket_name = urllib.quote_plus(bucket_name)
        object_name = urllib.quote_plus(object_name)
        request = {
    ...
    def post(self):
        ...
        if not backup_bucket or not path:
            logging.warning("Bad arguments, bucket: {}, path: {}".format(
                backup_bucket, path))
            self.abort(400)
        logging.info("Updating storage class for {}/{}".format(
            backup_bucket, path))
        # This uses the default service account for this application
        ...
        if storage_class == 'COLDLINE':
            ...
            return
        self.set_file_storage_class(backup_bucket, path, 'coldline',
                                    access_token)
...
class BigQueryImportEnqueue(BackupControllerBase):
    def get(self, backup_date):
        ...
        for entity in backup_entities:
            taskqueue.add(
                url='/backend-tasks/bigquery/import/{}/{}'.format(
                    backup_date, entity),
                queue_name='backups',
                method='GET')
...
class BigQueryImportEntity(BackupControllerBase):
    def get(self, backup_date, entity):
        ...
        logging.info("[{}] Loading {} into {}".format(job_name, source_url,
                                                      table))
        job = bigquery_client.load_table_from_storage(job_name, table,
                                                      source_url)
        ...
        while job.state != 'DONE' and wait_count > 0:
            logging.info("[{}] Job is {} ".format(datetime.datetime.now(),
                                                  job.state))
            time.sleep(10)
            job.reload()

        logging.info("[{}] Job ended at: {}".format(job_name, job.ended))
        logging.info("[{}] Job ended with state: {}".format(
            job_name, job.state))
        if job.errors:
            logging.error("Job error result: {}".format(job.error_result))
        else:
            logging.info("[{}] Job imported {} rows to bigquery".format(
                job_name, job.output_rows))
        if 'X-Appengine-Taskname' not in self.request.headers:
            self.response.out.write("[{}] Job ended with state: {}".format(
                job_name, job.state))
    ...
    def get(self, year=None):
        if year is None:
            ...
            self.response.out.write(
                "Enqueued backup for years: {}".format(years))
        else:
            event_keys = Event.query(Event.year == int(year)).fetch(
                None, keys_only=True)
            ...
            path = os.path.join(
                ...
                '../templates/backup/csv_backup_enqueue.html')
    ...
    def get(self, event_key):
        ...
        if event.awards:
            with cloudstorage.open(
                    self.AWARDS_FILENAME_PATTERN.format(
                        event.year, event_key, event_key), 'w') as awards_file:
                ...
                for award in event.awards:
                    for recipient in award.recipient_list:
                        ...
                        self._writerow_unicode(writer, [
                            award.key.id(), award.name_str, team,
                            recipient['awardee']
                        ])

        if event.matches:
            with cloudstorage.open(
                    self.MATCHES_FILENAME_PATTERN.format(
                        event.year, event_key, event_key),
                    'w') as matches_file:
                ...
                for match in event.matches:
                    ...
                    self._writerow_unicode(
                        writer,
                        [match.key.id()] + match.alliances['red']['teams'] +
                        match.alliances['blue']['teams'] +
                        [red_score, blue_score])

        if event.teams:
            with cloudstorage.open(
                    self.TEAMS_FILENAME_PATTERN.format(
                        event.year, event_key, event_key), 'w') as teams_file:
                writer = csv.writer(teams_file, delimiter=',')
                self._writerow_unicode(writer,
                                       [team.key.id() for team in event.teams])

        if event.rankings:
            with cloudstorage.open(
                    self.RANKINGS_FILENAME_PATTERN.format(
                        event.year, event_key, event_key),
                    'w') as rankings_file:
        ...
        if event.alliance_selections:
            with cloudstorage.open(
                    self.ALLIANCES_FILENAME_PATTERN.format(
                        event.year, event_key, event_key),
                    'w') as alliances_file:
...
class TbaCSVRestoreEventsEnqueue(webapp.RequestHandler):
    ...
    def get(self, year=None):
        if tba_config.CONFIG["env"] == "prod":  # disable in prod for now
            logging.error(
                "Tried to restore events from CSV for year {} in prod! No can do.".
                format(year))
            return
        ...
        if year is None:
            ...
            for y in years:
                taskqueue.add(
                    ...
                    method='GET')
            self.response.out.write(
                "Enqueued restore for years: {}".format(years))
        else:
            event_keys = Event.query(Event.year == int(year)).fetch(
                None, keys_only=True)
            ...
            for event_key in event_keys:
                taskqueue.add(
                    url='/tasks/do/csv_restore_event/{}'.format(
                        event_key.id()),
                    method='GET')
            ...
            path = os.path.join(
                os.path.dirname(__file__),
                '../templates/backup/csv_restore_enqueue.html')
            self.response.out.write(template.render(path, template_values))
...
class TbaCSVRestoreEventDo(webapp.RequestHandler):
    ...
    def get(self, event_key):
        if tba_config.CONFIG["env"] == "prod":  # disable in prod for now
            logging.error(
                "Tried to restore {} from CSV in prod! No can do.".format(
                    event_key))
            return
        ...
        # alliances
        result = urlfetch.fetch(
            self.ALLIANCES_URL.format(event.year, event_key, event_key))
        if result.status_code != 200:
            logging.warning('Unable to retreive url: ' + (
                self.ALLIANCES_URL.format(event.year, event_key, event_key)))
        else:
        ...
        # awards
        result = urlfetch.fetch(
            self.AWARDS_URL.format(event.year, event_key, event_key))
        if result.status_code != 200:
            logging.warning('Unable to retreive url: ' + (
                self.AWARDS_URL.format(event.year, event_key, event_key)))
        else:
            ...
            for row in csv.reader(
                    StringIO.StringIO(result.content), delimiter=','):
                writer.writerow([
                    event.year, event.event_short, row[1], row[2].replace(
                        'frc', ''), row[3]
                ])
        ...
        # matches
        result = urlfetch.fetch(
            self.MATCHES_URL.format(event.year, event_key, event_key))
        if result.status_code != 200:
            logging.warning('Unable to retreive url: ' + (
                self.MATCHES_URL.format(event.year, event_key, event_key)))
        else:
            data = result.content.replace('frc', '').replace(
                '{}_'.format(event_key), '')
        ...
        # rankings
        result = urlfetch.fetch(
            self.RANKINGS_URL.format(event.year, event_key, event_key))
        if result.status_code != 200:
            logging.warning('Unable to retreive url: ' + (
                self.RANKINGS_URL.format(event.year, event_key, event_key)))
        else:
            # convert into expected input format
            rankings = list(
                csv.reader(StringIO.StringIO(result.content), delimiter=','))
...
class TbaCSVBackupTeamsDo(webapp.RequestHandler):
    ...
    def get(self):
        team_keys_future = Team.query().order(
            Team.team_number).fetch_async(keys_only=True)
        social_media_keys_future = Media.query(Media.year == None).fetch_async(
            keys_only=True)

        team_futures = ndb.get_multi_async(team_keys_future.get_result())
        social_futures = ndb.get_multi_async(
            social_media_keys_future.get_result())
        ...
        for social_future in social_futures:
            ...
            for reference in social.references:
                socials_by_team[reference.id()][
                    social.media_type_enum] = social

        if team_futures:
            with cloudstorage.open(self.TEAMS_FILENAME_PATTERN,
                                   'w') as teams_file:
                writer = csv.writer(teams_file, delimiter=',')
                for team_future in team_futures:
                    team = team_future.get_result()
                    team_row = [
                        team.key.id(), team.nickname, team.name, team.city,
                        team.state_prov, team.country, team.website,
                        team.rookie_year
                    ]
                    for social_type in MediaType.social_types:
                        social = socials_by_team[team.key.id()].get(
                            social_type, None)
                        team_row.append(social.social_profile_url
