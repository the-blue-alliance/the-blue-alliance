...
TBA Admins
    ...
    def get(self):
        ...
        suggestions = [
            self._ids_and_events(suggestion) for suggestion in suggestions
        ]
        ...
        self.response.out.write(
            jinja2_engine.render(
                'suggestions/suggest_apiwrite_review_list.html',
    ...
    def post(self):
        ...
        if verdict == "accept":
            status = 'accept'
            auth_id, user, event_key, email_body = self._process_accepted(
                suggestion_id)
...
""".format(self.user_bundle.account.display_name,
           self.user_bundle.account.email, message, auth_id)
...
{}
""".format(self.user_bundle.account.display_name,
           self.user_bundle.account.email, message)

        # Notify the user their keys are available
        if email_body:
            mail.send_mail(
                sender=
                "The Blue Alliance Contact <contact@thebluealliance.com>",
                to=user.email,
                subject="The Blue Alliance Auth Tokens for {}".format(
                    event_key),
                body=email_body)
        if admin_email_body:
