...
class ApiBaseController(CacheableHandler):
    ...
    def delete_cache_multi(cls, cache_keys):
        ...
        ndb.delete_multi(
            [ndb.Key(CachedResponse, cache_key) for cache_key in cache_keys])
...
class ApiTrustedBaseController(webapp2.RequestHandler):
    ...
    def options(self, event_key):
        ...
        self.response.headers[
            'Access-Control-Allow-Headers'] = 'Content-Type, X-TBA-Auth-Id, X-TBA-Auth-Sig'
    ...
    def post(self, event_key):
        ...
        if not user_has_auth:
            ...
            expected_sig = md5.new(
                '{}{}{}'.format(auth.secret
                                if auth else None, self.request.path,
            ...
            if not auth or expected_sig != auth_sig:
                ...
                self._errors = json.dumps({
                    "Error":
                    "Invalid X-TBA-Auth-Id and/or X-TBA-Auth-Sig!"
                })
                self.abort(401)
