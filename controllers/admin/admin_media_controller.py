...
class AdminMediaDashboard(LoggedInHandler):
    ...
    def get(self):
        ...
        self.template_values.update({"media_count": media_count})

        path = os.path.join(
            os.path.dirname(__file__),
            '../../templates/admin/media_dashboard.html')
...
class AdminMediaMakePreferred(LoggedInHandler):
    def post(self, media_key_name):
        ...
        media.preferred_references.append(
            media.create_reference(
                self.request.get("reference_type"),
                self.request.get("reference_key_name")))
...
class AdminMediaRemovePreferred(LoggedInHandler):
    def post(self, media_key_name):
        ...
        media.preferred_references.remove(
            media.create_reference(
                self.request.get("reference_type"),
                self.request.get("reference_key_name")))
...
class AdminMediaInstagramImport(LoggedInHandler):
    def get(self):
        ...
        path = os.path.join(
            os.path.dirname(__file__),
            '../../templates/admin/media_import_instagram.html')
        self.response.out.write(template.render(path, self.template_values))
