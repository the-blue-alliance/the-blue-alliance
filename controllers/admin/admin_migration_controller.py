...
class AdminMigration(LoggedInHandler):
    def get(self):
        self._require_admin()
        path = os.path.join(
            os.path.dirname(__file__), '../../templates/admin/migration.html')
...
class AdminMigrationCreateEventDetails(LoggedInHandler):
    def get(self):
        ...
        for event_key in Event.query().fetch(keys_only=True):
            deferred.defer(
                create_event_details, event_key.id(), _queue="admin")
...
class AdminMigrationRankings(LoggedInHandler):
    def get(self, year):
        ...
        event_details = ndb.get_multi(
            [ndb.Key(EventDetails, key.id()) for key in event_keys])
        updated = []
        for event_detail in event_details:
            if event_detail:
                logging.info(event_detail.key.id())
                event_detail.rankings2 = RankingsHelper.convert_rankings(
                    event_detail)
