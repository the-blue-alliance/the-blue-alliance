...
class AdminOffseasonScraperController(LoggedInHandler):
    ...
    def get(self):
        ...
        truly_new_events = [
            ...
            if event.first_eid not in old_first_eids
        ]
        ...
        path = os.path.join(
            os.path.dirname(__file__), '../../templates/admin/offseasons.html')
    ...
    def post(self):
        ...
        if self.request.get("submit") == "duplicate":
            ...
            self.redirect("/admin/offseasons?success=duplicate&event_key=%s" %
                          self.request.get("duplicate_event_key"))
            return
        ...
        if self.request.get("submit") == "create":
            ...
            self.redirect(
                "/admin/offseasons?success=create&event_key=%s" % event_key)
            return
