...
class AdminAuthKeys(LoggedInHandler):
    ...
    def get(self):
        ...
        self.template_values.update({
            ...
            'twitch_secret':
            twitch_secrets.contents.get('client_id', "")
            if twitch_secrets else "",
            'livestream_secret':
            livestream_secrets.contents.get('api_key', "")
            if livestream_secrets else "",
        })

        path = os.path.join(
            os.path.dirname(__file__), '../../templates/admin/authkeys.html')
    ...
    def post(self):
        ...
        fmsapi_secrets.contents = {
            'username': fmsapi_user,
            'authkey': fmsapi_secret
        }
        mobile_clientIds.contents = {
            'web': web_client_id,
