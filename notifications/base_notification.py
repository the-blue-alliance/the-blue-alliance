...
class BaseNotification(object):
    ...
    _supported_clients = [
        ClientType.OS_ANDROID, ClientType.WEBHOOK, ClientType.WEB
    ]
    ...
    def send(self, keys, push_firebase=True, track_call=True):
        if self._timeout is not None:
            ...
            if memcache.get(
                    key
            ):  # Using memcache is a hacky implementation, since it is not guaranteed.
        ...
        deferred.defer(
            self.render, self._supported_clients, _queue="push-notifications")
    ...
    def _build_dict(self):
        raise NotImplementedError(
            "Subclasses must implement this method to build JSON data to send")

    @property
    def _type(self):
        raise NotImplementedError(
            "Subclasses must implement this message to set its notification type"
        )
    ...
    def track_notification(self, notification_type_enum, num_keys):
        ...
        if analytics_id is None:
            ...
            params = urllib.urlencode({
                ...
                GOOGLE_ANALYTICS_ID,
                'cid':
                uuid.uuid3(uuid.NAMESPACE_X500,
                           str('tba-notification-tracking')),
                't':
                'event',
                'ec':
                'notification',
                'ea':
                NotificationType.type_names[notification_type_enum],
                'ev':
                num_keys,
                'ni':
                1,
