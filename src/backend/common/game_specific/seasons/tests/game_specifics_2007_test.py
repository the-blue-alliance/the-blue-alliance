...
def test_ranking_sort_order_info() -> None:
    assert GameSpecifics2007().ranking_sort_order_info() == SORT_ORDER_INFO[2007]


def test_tiebreak_criteria_empty() -> None:
    assert GameSpecifics2007().tiebreak_criteria({}, {}) == []


def test_finals_can_be_tiebroken() -> None:
    assert GameSpecifics2007().finals_can_be_tiebroken() is False


def test_record_in_rankings() -> None:
    assert GameSpecifics2007().record_in_rankings() is True


def test_qual_average_in_rankings() -> None:
    assert GameSpecifics2007().qual_average_in_rankings() is False
