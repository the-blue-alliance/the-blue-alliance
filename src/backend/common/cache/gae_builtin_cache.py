...
class AppEngineBuiltinCache(CacheIf):
    ...
    def set_multi(self, mapping: Dict[bytes, Any], time: Optional[int] = None) -> None:
        self.memcache_client.set_multi(mapping, time or 0)

    def get(self, key: bytes) -> Optional[Any]:
        return self.memcache_client.get(key)

    def get_multi(self, keys: List[bytes]) -> Dict[bytes, Optional[Any]]:
