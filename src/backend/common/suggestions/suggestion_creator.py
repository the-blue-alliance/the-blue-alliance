...
class SuggestionCreator:
        ...
        if media_dict is not None:
            ...
            if existing_media is None or team_key not in [
                ...
                suggestion_id = Suggestion.render_media_key_name(
                    year_str, "team", team_key, foreign_type, media_dict["foreign_key"]
                )
                suggestion = Suggestion.get_by_id(suggestion_id)
                if (
                    not suggestion
                    or suggestion.review_state != Suggestion.REVIEW_PENDING
                ):
    ...
    def createEventMediaSuggestion(
        cls, author_account_key, media_url, event_key, private_details_json=None
    ):
    ...
    def createEventWebcastSuggestion(
        cls, author_account_key, webcast_url, webcast_date, event_key
    ):
    ...
    def createMatchVideoYouTubeSuggestion(
        cls, author_account_key, youtube_id, match_key
    ):
    ...
    def createApiWriteSuggestion(
        cls, author_account_key, event_key, affiliation, auth_types
    ):
        """
        ...
        if event_key:
            ...
            if event:
                ...
                clean_auth_types = filter(
                    lambda a: a in AuthType.write_type_names.keys(), auth_types
                )
                ...
                if event.event_type_enum in EventType.SEASON_EVENT_TYPES:
                    clean_auth_types = filter(
                        lambda a: a == AuthType.MATCH_VIDEO, clean_auth_types
                    )
