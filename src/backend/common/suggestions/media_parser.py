...
class MediaParser:
    ...
    def _create_media_dict(
        ...
        if media_type in PROFILE_URLS:
            media_dict["profile_url"] = PROFILE_URLS[media_type].format(foreign_key)
    ...
    def _parse_foreign_key(cls, media_type: MediaType, url: str) -> Optional[str]:
        ...
        logging.warning(
            "Failed to determine {} foreign_key from url: {}".format(
                TYPE_NAMES[media_type], url
            )
        )
        return None

    @classmethod
    def _sanitize_media_url(cls, media_type: MediaType, url: str) -> str:
        media_url = url.strip()
    ...
    def _partial_media_dict_from_cd_photo_thread(
        ...
        if image_partial is None:
            logging.warning(
                "Failed to determine image_partial from the page: {}".format(url)
            )
            return None
