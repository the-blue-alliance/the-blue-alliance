...
class InsightsStreaksYearQuery(CachedDatabaseQuery[List[Insight], List[InsightDict]]):
    CACHE_VERSION = 0
    CACHE_KEY_FORMAT = "insights_streaks_{year}"
    DICT_CONVERTER = InsightConverter
    ...
    def _query_async(self, year: Year) -> Generator[Any, Any, List[Insight]]:
        insight_names = []
        for insight_type in Insight.STREAK_INSIGHTS.keys():
            insight_names.append(Insight.INSIGHT_NAMES[insight_type])
