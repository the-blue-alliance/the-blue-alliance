...
class MatchTestCreator(object):
    ...
    def buildTestMatch(self, comp_level, set_number, match_number, complete):
        ...
        alliances = {
            "red": {"teams": [team.key_name for team in teams[:3]], "score": red_score},
            "blue": {
        ...
        if comp_level == "qm":
            id_string = "{}_{}{}".format(self.event.key_name, comp_level, match_number)
        else:
            id_string = "{}_{}{}m{}".format(
                self.event.key_name, comp_level, set_number, match_number
            )
        ...
        return Match(
            ...
            + datetime.timedelta(minutes=6) * match_number,
        )
    ...
    def createCompleteQuals(self):
        ...
        complete = True
        matches = [
            self.buildTestMatch(comp_level, set_number, match_number, complete)
    ...
    def createIncompleteQuals(self):
        ...
        matches = [
            self.buildTestMatch(comp_level, set_number, match_number, complete)
            for match_number in range(11, 21)
        ]
        MatchManipulator.createOrUpdate(matches)
        return matches
