...
def test_single_win_excluded(ndb_stub) -> None:
    ...
    _put_award(
        "2022nytr", 2022, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.REGIONAL
    )

    insights = compute_insights_for_year(2022, [MostWffaWinsV2Calculator()])
...
def test_two_wins_included(ndb_stub) -> None:
    ...
    _put_award(
        "2023nytr", 2023, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.REGIONAL
    )

    insights = compute_insights_for_year(0, [MostWffaWinsV2Calculator()])
...
def test_cmp_division_excluded(ndb_stub) -> None:
    _put_event("2022micmp1", 2022, EventType.CMP_DIVISION)
    _put_award(
        "2022micmp1", 2022, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.CMP_DIVISION
    )
    _put_event("2023micmp1", 2023, EventType.CMP_DIVISION)
    _put_award(
        "2023micmp1", 2023, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.CMP_DIVISION
    )
...
def test_cmp_finals_excluded(ndb_stub) -> None:
    _put_event("2022cmptx", 2022, EventType.CMP_FINALS)
    _put_award(
        "2022cmptx", 2022, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.CMP_FINALS
    )
    _put_event("2023cmptx", 2023, EventType.CMP_FINALS)
    _put_award(
        "2023cmptx", 2023, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.CMP_FINALS
    )
...
def test_non_wffa_award_does_not_count(ndb_stub) -> None:
    _put_event("2022nytr", 2022, EventType.REGIONAL)
...
def test_district_events_counted(ndb_stub) -> None:
    _put_event("2022micmp", 2022, EventType.DISTRICT_CMP)
    _put_award(
        "2022micmp", 2022, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.DISTRICT_CMP
    )
    _put_event("2023nytr", 2023, EventType.REGIONAL)
    _put_award(
        "2023nytr", 2023, ["frc254"], AwardType.WOODIE_FLOWERS, EventType.REGIONAL
...
def test_key_name(ndb_stub) -> None:
    ...
    assert insights[0].key_name == "0_v2_leaderboard_most_wffa_wins"
...
def test_insight_name_and_display_name(ndb_stub) -> None:
    ...
    assert insights[0].name == "most_wffa_wins"
    assert insights[0].display_name == "Most WFFAs"
