...
def test_single_week_average_margin(ndb_stub) -> None:
    _put_event("2022casj", 2022, start_date=_WEEK_1_START)
    _put_match("2022casj", 2022, 1, 100, 80)  # margin 20
    _put_match("2022casj", 2022, 2, 60, 40)  # margin 20

    insights = compute_insights_for_year(2022, [AverageWinMarginByWeekV2Calculator()])

    assert len(insights) == 1
    insight = insights[0]
    assert insight.name == "match_average_margins_by_week"
    ...
    assert len(points) == 1
    assert points[0]["x"] == 1
    assert points[0]["y"] == 20.0  # (20 + 20) / 2, no /2 for alliance count
...
def test_two_weeks_grouped_separately(ndb_stub) -> None:
    ...
    _put_match("2022caln", 2022, 1, 60, 10)  # margin 50
...
def test_elim_only_insight_uses_elim_matches_only(ndb_stub) -> None:
    _put_event("2022casj", 2022, start_date=_WEEK_1_START)
    _put_match("2022casj", 2022, 1, 100, 80, comp_level=CompLevel.QM)  # margin 20
    _put_match("2022casj", 2022, 1, 60, 10, comp_level=CompLevel.F)  # margin 50

    insights = compute_insights_for_year(2022, [AverageWinMarginByWeekV2Calculator()])

    assert len(insights) == 2
    all_insight = next(i for i in insights if i.name == "match_average_margins_by_week")
    elim_insight = next(
        i for i in insights if i.name == "elim_match_average_margins_by_week"
    )

    all_points = all_insight.data["series"][0]["points"]
    assert all_points[0]["y"] == 35.0  # (20 + 50) / 2

    elim_points = elim_insight.data["series"][0]["points"]
    assert len(elim_points) == 1
    assert elim_points[0]["y"] == 50.0
...
def test_no_elim_matches_produces_no_elim_insight(ndb_stub) -> None:
    ...
    assert len(insights) == 1
    assert insights[0].name == "match_average_margins_by_week"
...
def test_championship_event_lands_on_trailing_week(ndb_stub) -> None:
    ...
    _put_match("2022cmptx", 2022, 1, 200, 120)  # margin 80
