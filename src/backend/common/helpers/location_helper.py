...
class LocationHelper:
    ...
    @classmethod
    def get_similarity(cls, a: str, b: str) -> float:
        """
    ...
    def update_event_location(cls, event: Event) -> None:
        ...
        if not location_info:
            logging.warning(
                "Falling back to location only for event {}".format(event.key.id())
            )
            geocode_result = cls.google_maps_geocode(event.location)
            if geocode_result:
                location_info = cls.construct_location_info(geocode_result[0])
            else:
    ...
    def get_event_location_info(cls, event: Event) -> Tuple[LocationInfo, float]:
        ...
        for j, query in enumerate(possible_queries):
            # Try both searches
            nearbysearch_places = cls.google_maps_placesearch(query, lat_lng)
            textsearch_places = cls.google_maps_placesearch(
                query, lat_lng, textsearch=True
            )

            for results_future in [nearbysearch_places, textsearch_places]:
                for i, place in enumerate(results_future[:5]):
    ...
    @classmethod
    def compute_event_location_score(
        cls, query_name: str, location_info: LocationInfo, lat_lng: LatLng
    ) -> float:
        """
        ...
        if (
            "Shenzhen" in query_name
            and location_info["name"] != "Shenzhen University Town Sports Center"
        ):
            return 0
    ...
    def compute_team_location_score(
        ...
        result_name = (
            location_info["name"].lower().replace("school", "").replace("high", "")
        )
    ...
    def construct_location_info(
        ...
        place_details_result = cls.google_maps_place_details(gmaps_result["place_id"])
        if place_details_result:
            ...
            if auto_fill:
                if (
                    "geometry" in place_details_result
                    and "location" in place_details_result["geometry"]
                ):
                    location_info["lat"] = place_details_result["geometry"]["location"][
                        "lat"
                    ]
                    location_info["lng"] = place_details_result["geometry"]["location"][
                        "lng"
                    ]
    ...
    def google_maps_placesearch(
        ...
        results = None
        if query:
            cache_key = "google_maps_{}:{}".format(search_type, query).encode()
            ...
            if results is None:
                ...
                search_url = (
                    "https://maps.googleapis.com/maps/api/place/{}/json".format(
                        search_type
                    )
                )
                try:
                    ...
                    if search_result.status_code == 200:
                        ...
                        if search_dict["status"] == "ZERO_RESULTS":
                            logging.info(
                                "No {} results for query: {}, lat_lng: {}".format(
                            ...
                            logging.warning(
                                "{} failed with query: {}, lat_lng: {}".format(
                                    search_type, query, lat_lng
                                )
                            )
    ...
    def google_maps_place_details(cls, place_id: str) -> Optional[Dict]:
        ...
        cache_key = "google_maps_place_details:{}".format(place_id).encode()
        memcache = MemcacheClient.get()
        ...
        if result is None:
            ...
            try:
                ...
                logging.warning(
                    "urlfetch for place_details request failed with place_id: {}.".format(
                        place_id
                    )
                )
                logging.warning(e)
    ...
    def google_maps_geocode(cls, location: Optional[str]) -> List[Dict]:
        ...
        if not results:
            ...
            try:
                ...
                logging.warning(
                    "urlfetch for geocode request failed for location {}.".format(
                        location
                    )
                )
                logging.warning(e)
    ...
    def get_timezone_id(
        ...
        try:
            tz_result = requests.get(tz_url, params=tz_params)
        except Exception as e:
        ...
        if tz_result.status_code != 200:
            logging.warning(
                "TZ lookup for (lat, lng) failed! ({}, {})".format(lat, lng)
            )
            return None
        tz_dict = tz_result.json()
        if "timeZoneId" not in tz_dict:
            logging.warning(f"No timeZoneId for ({lat}, {lng})")
            return None
