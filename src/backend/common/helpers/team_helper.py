    ...
    def getPopularTeamsEvents(
        ...
        event_teams_futures = [
            (
                ...
                ).fetch_async(projection=[EventTeam.team]),
            )
        ...
        memcache.set_multi(
            team_favorite_counts, 60 * 60 * 24, namespace="team-favorite-counts"
        )

        # Sort to get top popular teams
        popular_team_keys = []
        for team_key, _ in sorted(team_favorite_counts.items(), key=lambda tc: -tc[1])[
            :25
        ]:
            popular_team_keys.append(ndb.Key(Team, team_key))
        popular_teams = sorted(
            ndb.get_multi(popular_team_keys), key=lambda team: team.team_number
        )
