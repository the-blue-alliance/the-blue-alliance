...
class AverageWinMarginByWeekV2Calculator(TimeseriesV2Calculator):
    """
    Tracks the average match winning margin by week, for a single year.
    Produces two insights: one across all matches, one across elim matches
    only. Championship events (which have no numeric week) are folded into a
    trailing synthetic week after the last regular week.
    """
    ...
    def insight_name(self) -> InsightV2NameEntry:
        return InsightV2Names.AVERAGE_WIN_MARGIN_BY_WEEK
    ...
    def on_event(self, event: Event) -> None:
        ...
        for match in event.matches or []:
            ...
            if is_cmp:
                self._cmp_sum += margin
                self._cmp_count += 1
                if is_elim:
                    self._elim_cmp_sum += margin
                    self._elim_cmp_count += 1
            else:
                w = week + 1  # type: ignore[operator]
                self._week_sums[w] += margin
                self._week_counts[w] += 1
                if is_elim:
                    self._elim_week_sums[w] += margin
    ...
    def _build_points(
        ...
        points: List[TimeseriesPointNoContext] = [
            TimeseriesPointNoContext(
                x=week, y=float(week_sums[week]) / week_counts[week]
            )
            for week in sorted(week_counts.keys())
        ]
        if cmp_count:
            points.append(
                TimeseriesPointNoContext(x=trailing_week, y=float(cmp_sum) / cmp_count)
            )
        return points
    ...
    def make_insights(
        ...
        if all_points:
            name = InsightV2Names.AVERAGE_WIN_MARGIN_BY_WEEK
            insights.append(
                InsightV2(
        ...
        if elim_points:
            name = InsightV2Names.AVERAGE_ELIM_WIN_MARGIN_BY_WEEK
            insights.append(
                InsightV2(
                    id=InsightV2.render_key_name(
                        year, InsightCategory.TIMESERIES, name.name
