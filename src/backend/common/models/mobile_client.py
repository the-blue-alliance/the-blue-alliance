...
class MobileClient(ndb.Model):
    ...
    client_type = ndb.IntegerProperty(required=True, choices=list(ClientType))
    ...
    def type_string(self):
        return client_type.NAMES[self.client_type]
    ...
    def short_id(self):
        return (
            self.messaging_id
            if len(self.messaging_id) <= 50
            else self.messaging_id[0:50] + "..."
        )
