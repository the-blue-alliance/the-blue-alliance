...
class EventTeamsNotification(Notification):
    def __init__(self, event: Event, added_teams: List[Team], removed_teams: List[Team]) -> None:
        self.event = event
    ...
    def webhook_message_data(self) -> Optional[Dict[str, Any]]:
        ...
        payload["added_teams"] = [TeamConverter.teamConverter_v3(team) for team in self.added_teams]
        payload["removed_teams"] = [TeamConverter.teamConverter_v3(team) for team in self.removed_teams]
        return payload
