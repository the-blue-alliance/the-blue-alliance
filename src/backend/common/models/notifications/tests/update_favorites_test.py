...
from backend.common.models.notifications.mytba import FavoritesUpdatedNotification
...
def test_type() -> None:
    assert FavoritesUpdatedNotification._type() == NotificationType.UPDATE_FAVORITES


def test_payload() -> None:
    notification = FavoritesUpdatedNotification("user_id", "device_id")
...
def test_collapse_key() -> None:
    ...
    assert platform_config.collapse_key == "user_id_update_favorites"
...
def test_notification_shoud_skip(
    user_id: str,
    notif_device_id: str | None,
    client_type: ClientType,
    client_device_id: str,
    should_send: bool,
) -> None:
    notification = FavoritesUpdatedNotification(user_id, notif_device_id)
