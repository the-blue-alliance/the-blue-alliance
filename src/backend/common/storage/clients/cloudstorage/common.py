...
_GCS_OPTIONS = _GCS_METADATA + ["x-goog-acl", "x-goog-if-generation-match"]
...
def validate_bucket_name(name):
    ...
    if len(name) < _GCS_BUCKET_NAME_MIN_LEN:
        raise ValueError(
            "GCS Buckets must have at least %d characters, "
    ...
    if len(name) > _GCS_BUCKET_NAME_MAX_LEN:
        raise ValueError(
            "GCS Buckets may have no more than %d characters, "
            "but you specified '%s'. See "
            "https://cloud.google.com/storage/docs/naming#requirements"
...
def posix_to_dt_str(posix):
    ...
    Returns:
      A datetime str.
    """
    dt = datetime.datetime.fromtimestamp(posix, datetime.UTC)
...
def local_api_url():
    ...
    return "http://%s%s" % (context.get("HTTP_HOST"), LOCAL_GCS_ENDPOINT)
