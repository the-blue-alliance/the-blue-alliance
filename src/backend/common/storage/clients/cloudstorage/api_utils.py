...
def _quote_filename(filename, encoding=None):
    """Quotes filename to use as a valid URI path.

    Args:
      filename: user provided filename. /bucket/filename.
      encoding: The encoding used to convert filename to bytes, defaults to utf-8
                in urllib.parse.quote function and must be None if filename is
                bytes.

    Returns:
      The filename properly quoted to use as URI's path component.
    """
    # The entire path string is quoted, including the bucket, even though the
    # method claims to only encode the filename. This is OK because bucket name
    # only has unreserved chars so escaping will have no effect on it.
    if six.PY2:
        return six.moves.urllib.parse.quote(filename)
    else:
        return six.moves.urllib.parse.quote(filename, encoding=encoding)


def _unquote_filename(filename, encoding="utf-8"):
    ...
    Args:
      ...
      encoding: The encoding of the encoded filename, default to utf-8. for
                backward compatibility with py2 set to latin-1.
                Unquote function decodes the string to latin-1 encoding:
                http://google3/third_party/python_runtime/v2_7/Lib/urllib.py?l=1231&rcl=258535945
    Returns:
    ...
    if six.PY2:
        ...
        return six.moves.urllib.parse.unquote(filename, encoding=encoding)


def _should_retry(resp):
    """Given a urlfetch response, decide whether to retry that request."""
    return resp.status_code == http_client.REQUEST_TIMEOUT or (
        resp.status_code >= 500 and resp.status_code < 600
    )
...
class _RetryWrapper(object):
    ...
    def run(self, tasklet, **kwds):
        ...
        while True:
            # Clear out error and result from previous runs.
            e = None
            result = None
            got_result = False
            ...
            try:
                ...
                got_result = True
                # Raise back the result if no retry is needed.
                if not self.should_retry(result):
                    raise ndb.Return(result)
            # This is the error from RUNTIME.
...
class RetryParams(object):
    ...
    def __init__(
        ...
        initial_delay=0.1,
        max_delay=10.0,
        min_retries=3,  # 4 attempts total
        max_retries=6,  # 7 attempts total
        ...
        self.save_access_token = self._check(
            "save_access_token", save_access_token, True, bool
        )
        self.memcache_access_token = self._check(
            "memcache_access_token", memcache_access_token, True, bool
        )
    ...
    def delay(self, n, start_time):
        ...
        if n > self.max_retries or (
            ...
            return -1
        # TODO(b/72688219): GCS suggests randomized binary exponential backoff.
        # Add random factor if needed.
        return min(
            math.pow(self.backoff_factor, n - 1) * self.initial_delay, self.max_delay
        )
