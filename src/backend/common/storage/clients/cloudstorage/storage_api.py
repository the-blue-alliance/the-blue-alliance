...
from backend.common.storage.clients.cloudstorage import api_utils
from backend.common.storage.clients.cloudstorage import common
from backend.common.storage.clients.cloudstorage import errors
from backend.common.storage.clients.cloudstorage import rest_api

# pylint: disable=protected-access


def _get_storage_api(retry_params, token_params):
    ...
    Returns:
      ...
      to the real GCS.
    """
    # When running on dev appserver or in unittest env, the storage_api
    # instance is configured to talk to local storage stub. If
    # common._access_token is specified, storage_api will use it as
    # authentication token and talk to real storage regardless of environment.

    # The StorageAPI is intended for internal use only, not external users
    # pylint: disable=protected-access
    api = _StorageApi(
        _StorageApi.full_control_scope,
...
class _StorageApi(rest_api._RestApi):
    ...
    def do_request_async(
        ...
        # Tell GFE not to gunzip or do any other transformation. Just
        # give us the original content as is.
    ...
    # These methods are tasklets even though they are not marked as
    # such; they wrap a lower-level tasklet method without adding an
    # extra layer of Futures.
...
class ReadBuffer(object):
    ...
    # 30MB. urlfetch response limit is 32MB.
    ...
    def __init__(
        ...
        self._api = api
        self._path = path
        # Public property according to Python Files interface.
        ...
        # _buffer_future is used by read to get the next buffer before read is
        # actually called. Thus, any method that updates _offset should invalidate
        # this future and request a new one. See self._request_next_buffer.
        self._buffer_future = None

        # Use get_future to fill self._buffer if file is not empty.
        # Range header uses inclusive offset so when file is empty,
        # even Range: 0-0 will get an error.
    ...
    def read(self, size=-1):
        ...
        while True:
            ...
            if size >= 0 and size < remaining:
                ...
                # Read everything from buffer first.
                size -= remaining
                self._offset += remaining
                data_list.append(self._buffer.read())

                # If we already consumed a future and still don't have enough, fire
                # multiple RPCs in parallel and fetch as much as we can instead of
                # repeatedly fetching with fixed size.
                ...
                # Read from future if there is one.
    ...
    def _get_segment(self, start, request_size, check_response=True):
        ...
        """
        # -1 because Google storage uses inclusive offsets.
        ...
        def _checker():
            errors.check_status(
                status, [200, 206], self._path, headers, resp_headers, body=content
            )
...
class StreamingBuffer(object):
    ...
    _blocksize = 256 * 1024  # Google Storage *requires* 256K-aligned writes.

    # Buffer size to trigger a flush to GCS.
    _flushsize = 8 * _blocksize  # 2MB.

    _maxrequestsize = 9 * 4 * _blocksize  # 9MB (urlfetch request limit is 10MB)
    ...
    def __init__(self, api, path, content_type=None, gcs_headers=None):
        ...
        """
        # Check that the limits are reasonable (dynamic, because unit
        # tests may reset these).
        ...
        # Set up buffer bookkeeping.
        self._buffer = collections.deque()  # Deque of byte strings.
        self._buffered = 0  # Equals sum(len(b) for b in self._buffer).
        self._written = 0  # Number of bytes written to GCS so far.
        self._offset = 0  # Byte offset for tell().
        ...
        # Compute self._path_with_token, needed to write data blocks.
    ...
    def _flush(self, finish=False):
        ...
        while (finish and self._buffered >= 0) or (
            ...
            tmp_buffer = []  # pylint: disable=redefined-builtin
            tmp_buffer_len = 0

            # Transfer strings from the front of self._buffer to buffer
            # until we have more than self._maxrequestsize or we have too little
            # left in self._buffer to flush another block.
            excess = 0
            while self._buffer:
                ...
                if tmp_buffer_len >= self._maxrequestsize:
                    ...
                    break
                # After transferring the last "buf", we got enough in tmp buffer
                # to flush but too little in internal buffer to form another block.
                if not finish and (
                    tmp_buffer_len % self._blocksize + self._buffered < self._blocksize
                ):
                    excess = tmp_buffer_len % self._blocksize
                    break

            # Push back excess data.
            if excess:
                over = tmp_buffer.pop()
                size = len(over)
                assert size >= excess
    ...
    def _get_offset_from_gcs(self):
        ...
        # range, if exists, should have format bytes=0-LastOffset.
