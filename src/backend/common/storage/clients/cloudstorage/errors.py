...
class InsufficientQuotaError(TransientError):
...
def check_status(
    ...
    Raises:
      ...
      InsufficientQuotaError: if the bucket is out of any quota.
    ...
    if status == http_client.UNAUTHORIZED:
        raise AuthorizationError(msg)
    elif status == http_client.FORBIDDEN:
        raise ForbiddenError(msg)
    elif status == http_client.NOT_FOUND:
        raise NotFoundError(msg)
    elif status == http_client.BAD_REQUEST:
        raise BadRequestError(msg)
    elif status == http_client.BAD_REQUEST:
        raise BadRequestError(msg)
    elif status == http_client.REQUEST_TIMEOUT:
        raise TimeoutError(msg)
    elif status == http_client.REQUESTED_RANGE_NOT_SATISFIABLE:
        raise InvalidRange(msg)
    elif status == 429:
        raise InsufficientQuotaError(msg)
    elif status == http_client.OK and 308 in expected:
