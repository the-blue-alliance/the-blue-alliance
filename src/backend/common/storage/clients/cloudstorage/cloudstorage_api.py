...
from backend.common.storage.clients.cloudstorage import storage_api
...
def open(
    ...
    Args:
      ...
      options: A str->basestring dict to specify additional headers to pass to
        ...
        Supported options are x-goog-acl, x-goog-meta-,
        x-goog-if-generation-match, cache-control, content-disposition,
    ...
    if mode == "wb":
        ...
        return storage_api.ReadBuffer(
            api, filename, buffer_size=read_buffer_size, offset=offset
        )
    else:
...
def delete_async(filename, retry_params=None, token_params=None):
    ...
    errors.check_status(
        status, [204], filename, resp_headers=resp_headers, body=content
    )
...
@ndb.tasklet
def _get_bucket_attribute_async(
    bucket, query_param, xml_response_tag, retry_params=None, token_params=None
):
    ...
    status, headers, content = yield api.get_bucket_async(
        "%s?%s" % (bucket, query_param)
    )
...
def compose(
    ...
    """
    api = storage_api._get_storage_api(retry_params=retry_params, token_params=None)
    ...
    file_list, _ = _validate_compose_list(
        destination_file, list_of_files, files_metadata, 32
    )
...
def _validate_compose_list(
    destination_file, file_list, files_metadata=None, number_of_files=32
):
    ...
    if files_metadata is None:
        ...
        raise ValueError(
            "files_metadata contains more entries(%i)"
            " than file_list(%i)" % (len(files_metadata), list_len)
        )
    list_of_files = []
    for source_file, meta_data in six.moves.zip_longest(file_list, files_metadata):
        if not isinstance(source_file, str):
            raise TypeError("Each item of file_list must be a string")
        if source_file.startswith("/"):
            logging.warning(
                'Detected a "/" at the start of the file, '
                'Unless the file name contains a "/" it '
                " may cause files to be misread"
            )
        if source_file.startswith(bucket):
            logging.warning(
                "Detected bucket name at the start of the file, "
