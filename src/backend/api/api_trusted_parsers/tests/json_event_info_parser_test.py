...
def _make_parser(
    start_date: datetime | None = datetime(2024, 3, 14),
    end_date: datetime | None = datetime(2024, 3, 16),
) -> JSONEventInfoParser:
    return JSONEventInfoParser(Event(start_date=start_date, end_date=end_date))


def test_parse_empty_dict() -> None:
    parsed = _make_parser().parse("{}")
    assert parsed == {}


def test_parse_bytes_input() -> None:
    data = {"first_event_code": "CASJ"}
    parsed = _make_parser().parse(json.dumps(data).encode("utf-8"))
...
def test_parse_webcast_with_type_and_channel() -> None:
    ...
    parsed = _make_parser().parse(json.dumps(data))
...
def test_parse_webcast_youtube_without_date_raises_exception() -> None:
    ...
    with pytest.raises(
        ParserInputException, match="YouTube webcasts must have the 'date' field set"
    ):
        _make_parser().parse(json.dumps(data))
...
def test_parse_webcast_date_outside_event_range_raises_exception() -> None:
    data = {
        "webcasts": [
...
def test_parse_webcast_date_on_event_start_date_succeeds() -> None:
    data = {
        "webcasts": [
            {"type": "twitch", "channel": "firstinspires", "date": "2024-03-14"}
        ]
    }
    parsed = _make_parser(
        start_date=datetime(2024, 3, 14), end_date=datetime(2024, 3, 16)
    ).parse(json.dumps(data))
    assert none_throws(parsed.get("webcasts"))[0].get("date") == "2024-03-14"


def test_parse_webcast_date_on_event_end_date_succeeds() -> None:
    data = {
        "webcasts": [
            {"type": "twitch", "channel": "firstinspires", "date": "2024-03-16"}
        ]
    }
    parsed = _make_parser(
        start_date=datetime(2024, 3, 14), end_date=datetime(2024, 3, 16)
    ).parse(json.dumps(data))
    assert none_throws(parsed.get("webcasts"))[0].get("date") == "2024-03-16"


def test_parse_webcast_date_skips_range_check_without_event_dates() -> None:
    # Date range validation is skipped when the event has no start/end dates
    data = {
        "webcasts": [
            {"type": "twitch", "channel": "firstinspires", "date": "2099-01-01"}
        ]
    }
    parsed = _make_parser(start_date=None, end_date=None).parse(json.dumps(data))
    assert none_throws(parsed.get("webcasts"))[0].get("date") == "2099-01-01"
