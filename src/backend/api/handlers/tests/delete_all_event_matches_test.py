...
def test_bad_event_key(api_client: Client) -> None:
    ...
    resp = api_client.post(
        "/api/trusted/v1/event/asdf/matches/delete_all", data=json.dumps([])
    )
...
def test_bad_event(api_client: Client) -> None:
    ...
    resp = api_client.post(
        "/api/trusted/v1/event/2015casj/matches/delete_all", data=json.dumps([])
    )
    assert resp.status_code == 404
...
def test_bad_auth_type(api_client: Client) -> None:
    ...
    resp = api_client.post(
        "/api/trusted/v1/event/2014casj/matches/delete_all", data=json.dumps([])
    )
...
def test_delete_all_bad_body(api_client: Client) -> None:
    setup_event()
    setup_matches(n=3)
    setup_auth(access_types=[AuthType.EVENT_MATCHES])

    request_body = "doesnt_match"
    response = api_client.post(
        REQUEST_PATH,
        headers=get_auth_headers(REQUEST_PATH, request_body),
        data=request_body,
...
def test_delete_all_matches(api_client: Client) -> None:
    setup_event()
    setup_matches(n=5)
    setup_auth(access_types=[AuthType.EVENT_MATCHES])
