...
from backend.tasks_io.helpers.tbans_helper import _firebase_app, TBANSHelper
from backend.tasks_io.models.notification_requests import FCMRequest, WebhookRequest
from backend.tasks_io.models.notifications import (
...
class TestTBANSHelper(unittest.TestCase):
    ...
    @patch.object(TBANSHelper, "_send")
    def test_alliance_selection_no_users(self, mock_send):
        # Test send not called with no subscribed users
        TBANSHelper.alliance_selection(self.event)
        mock_send.assert_not_called()

    @patch.object(TBANSHelper, "_send")
    def test_alliance_selection_user_id(self, mock_send):
    ...
    @patch.object(TBANSHelper, "_send")
    def test_alliance_selection(self, mock_send):
    ...
    @patch.object(TBANSHelper, "_send")
    def test_awards_no_users(self, mock_send):
        # Test send not called with no subscribed users
        TBANSHelper.awards(self.event)
        mock_send.assert_not_called()

    @patch.object(TBANSHelper, "_send")
    def test_awards_user_id(self, mock_send):
        # Test send called with user id
    ...
    @patch.object(TBANSHelper, "_send")
    def test_awards(self, mock_send):
    ...
    @patch("backend.tasks_io.models.notification_requests.FCMRequest", autospec=True)
    @patch.object(FCMRequest, "send")
    def test_send_fcm_filter_fcm_clients(self, mock_send, mock_init):
        expected = [
    ...
    def test_send_fcm_batch(self):
        ...
        batch_response = messaging.BatchResponse([])
        with patch(
            "backend.tasks_io.models.notification_requests.fcm_request.MAXIMUM_TOKENS",
            2,
    ...
    @patch(
        "backend.tasks_io.models.notification_requests.WebhookRequest", autospec=True
    )
    def test_send_webhook_filter_webhook_clients(self, mock_init):
    ...
    def test_send_webhook_filter_webhook_clients_verified(self, mock_init):
        clients = [
            MobileClient(
                parent=ndb.Key(Account, "user_id"),
                user_id="user_id",
