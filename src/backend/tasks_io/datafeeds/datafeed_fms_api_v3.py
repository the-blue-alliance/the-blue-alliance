...
from backend.tasks_io.datafeeds.models.fms_api.fms_api import FMSAPI

# TODO: I don't think this inherits from datafeed base honestly

# TODO: Probably just rewrite the old datafeed and drop the v3 here
class DatafeedFMSAPIV3:
    ...
    FMS_API_URL_BASE = "https://frc-api.firstinspires.org/v3.0"
    # TODO: Update for v3?
    SAVED_RESPONSE_DIR_PATTERN = '/tbatv-prod-hrd.appspot.com/frc-api-response/{}/'  # % (url)

    # TODO: Simtime?
    def __init__(self, sim_time=None, save_response=False):
        auth_token = FMSApiSecrets.auth_token()
        if not auth_token and sim_time is None:
            raise Exception(f"Missing FMS API auth token. Setup {FMSApiSecrets.key()} sitevar.")

        self.session = requests.Session()
        self.session.headers.update({"Authorization": f"Basic {auth_token}"})

        # TODO: Set some basic domain here
    ...
    # TODO: Add type here for response
    def get_index(self) -> FMSAPI:
        # {
        #   "currentSeason": 2021,
        #   "maxSeason": 2021,
        ...
        #   "status": "normal"
        # }
        # Note: The index route does not require auth - so we won't call it with our auth headers
        resp = requests.get(self.FMS_API_URL_BASE + "/").json()
        # Convert keys from camelCase to snake_case
        resp = dict((re.sub("([A-Z]+)", r"_\1", k).lower(), v) for k, v in resp.items())
        # Drop the fields we don't care about...
        resp = dict((k, v) for k, v in resp.items() if k not in ['service_manifest_name', 'service_manifest_version', 'code_package_name', 'code_package_version'])
        return FMSAPI(**resp)

    def get_season(self, season: int):
        return self.session.get(self.FMS_API_URL_BASE + f"/{str(season)}").json()

    # TODO: week_number, tournament_type
    def get_events(self, season: int, event_code: str, team_number: int, district_code: str, exclude_district: List[str]):
        # Note: `event_code` and `team_number` read to be mutually exclusive
        # Note: `event_code`, `district_code`, and `exclude_district` are mutually exclusive
        # Basically - `district_code` and `exclude_district` are used with `team_number`
        # Note: In 2015, `divisionCode` will exist on some events
        # `type: ChampionshipDivision` cannot request match results, rankings, schedules or alliances. Only awards.
        #
        # District Codes: FMA PNW NE FIN FNC ONT ISR CHS FIT PCH FIM (will probably change)
        # Tournament Codes:
        # 1. None | 2. Regional | 3. DistrictEvent | 4. DistrictChampionship | 5. DistrictChampionshipWithLevels
        # 6. DistrictChampionshipDivision | 7. ChampionshipSubdivision | 8. ChampionshipDivision | 9. Championship
        # 10. OffSeason | 11. OffSeasonWithAzureSync
        pass
