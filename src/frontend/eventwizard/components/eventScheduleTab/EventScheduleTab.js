...
function EventScheduleTab({ selectedEvent, makeTrustedRequest }) {
  ...
  const handleFilterChange = async (event) => {
    ...
    // Re-parse file with new filter if file is loaded
    if (file) {
      setLoading(true);
      ...
      try {
        ...
        if (parsedMatches.length > 0) {
          ...
          setStatusMessage("No matches found with current filter.");
        }
      } catch (error) {
        setStatusMessage(`Error parsing file: ${error.message}`);
        setMatches([]);
  ...
  const handleSubmit = () => {
    ...
    makeTrustedRequest(
      ...
      () => {
        setUploading(false);
        alert(`Successfully uploaded ${matches.length} matches!`);
      },
      (error) => {
        setUploading(false);
        alert(`Error uploading matches: ${error}`);
      }
    );
  };
