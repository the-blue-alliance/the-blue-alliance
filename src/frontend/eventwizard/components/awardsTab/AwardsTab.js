...
function AwardsTab({selectedEvent}) {
  ...
  return <div className="tab-pane" id="awards">
    ...
    <h4>Add Award</h4>
    <p>If an award type is not specified, we will automatically try to determine it from the award name. Note that this is not guaranteed to work, especially for offseason events.</p>
    ...
    <button className="btn btn-info" onClick={addAward} disabled={updating}>Add Award</button>
        ...
        {sortedAwards.map(({type_enum, name_str, team_key, awardee}) => {
          ...
          return (
            <tr key={awardKey}>
              ...
              <td>{awardee}</td>
              <td>
                <button className={toDelete ? "btn btn-danger" : "btn"} onClick={() => toggleDeletion(awardKey)}>
    ...
    <p>Please ensure that you have fetched existing awards before submitting!</p>
    <button
