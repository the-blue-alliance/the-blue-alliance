...
import { parseResultsFile, cleanTeamNum } from "../../utils/resultsParser";

function EventMatchResultsTab({ selectedEvent, makeTrustedRequest }) {
  const [file, setFile] = useState(null);
  const [matches, setMatches] = useState([]);
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [hasOcto, setHasOcto] = useState(false);
  ...
  const handleAllianceCountChange = async (event) => {
  ...
  const handlePlayoffFormatChange = async (newIsDoubleElim) => {
  ...
  return (
    <div className="tab-pane" id="results">
      ...
      <p>
        Lets you upload a FMS Match Results report. Note that this will
        overwrite data that currently exists for these matches.
      </p>
      ...
      {matches.length > 0 && (
        <div className="row">
          <div className="col-sm-12">
            <div className="panel panel-default">
              <div className="panel-heading">
                <h4 className="panel-title">Match Results Preview</h4>
              </div>
              <div className="panel-body">
                ...
                <button
                  ...
                  disabled={uploading}
                >
                  {uploading ? "Uploading..." : "Upload Match Results to TBA"}
                </button>
              </div>
            </div>
          </div>
