...
const trimToLayout = (state) => {
  ...
  const maxViewsForLayout = LayoutConstants_1.NUM_VIEWS_FOR_LAYOUT[layoutId];
  ...
  // Pad the end with -1s
  while (positionMap.length < LayoutConstants_1.MAX_SUPPORTED_VIEWS) {
    positionMap.push(-1);
  }
  // Fill in any empty spots in the dom order
  while (domOrder.length < LayoutConstants_1.MAX_SUPPORTED_VIEWS) {
    domOrder.push(null);
...
const addWebcastAtPosition = (
  ...
  if (
    !state.layoutSet ||
    position >= LayoutConstants_1.NUM_VIEWS_FOR_LAYOUT[state.layoutId]
  ) {
    return state;
  }
