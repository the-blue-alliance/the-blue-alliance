...
class FavoriteMessage(messages.Message):
    ...
    device_key = messages.StringField(
        2)  # So we know which device NOT to push sync notification to
...
class SubscriptionMessage(messages.Message):
    ...
    device_key = messages.StringField(
        3)  # So we know which device NOT to push sync notifications to
    model_type = messages.IntegerField(4, required=True)


class SubscriptionCollection(messages.Message):
    subscriptions = messages.MessageField(
        SubscriptionMessage, 1, repeated=True)
