...
class Award2(ndb.Model):
    ...
    name_str = ndb.StringProperty(required=True, indexed=False)
    ...
    team_list = ndb.KeyProperty(kind=Team, repeated=True)  # team(s) that won the award (if applicable)
    recipient_json_list = ndb.StringProperty(repeated=True)  # JSON dict(s) with team number and/or awardee
