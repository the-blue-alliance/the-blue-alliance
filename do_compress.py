...
SCRIPTS_GAMEDAY = SCRIPTS_MAIN + [
    'static/javascript/tba_js/gameday.js',
...
SCRIPTS_EVENTWIZARD = [
    'static/javascript/tba_js/eventwizard_apiwrite.js',
...
def compress_js(in_files, out_file):
    os.system('uglifyjs {} --compress -o {}'.format(' '.join(in_files),
                                                    out_file))
    print '=> %s' % out_file
    print ''
...
def main(kind=None):
    ...
    if kind == 'js' or kind is None:
        ...
        compress_js(SCRIPTS_FIREBASE_SERVICEWORKER,
                    SCRIPTS_FIREBASE_SERVICEWORKER_OUT)
