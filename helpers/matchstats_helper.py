...
class MatchstatsHelper(object):
    ...
    def build_s_matrix(cls,
        ...
        for match in matches:
            ...
            for alliance_color in ['red', 'blue']:
                ...
                stat = cls._get_stat(stat_type, match, alliance_color,
                                     alliance_teams, init_stats,
                                     init_stats_default, treat_as_unplayed)
    ...
    @classmethod
    def _get_stat(cls, stat_type, match, alliance_color, alliance_teams,
                  init_stats, init_stats_default, treat_as_unplayed):
    ...
    def calculate_matchstats(cls, matches, year):
        ...
        if not team_list:
            return {}
        last_event_stats = cls.get_last_event_stats(team_list,
                                                    matches[0].event)

        Minv = cls.build_Minv_matrix(matches, team_id_map, played_only=True)

        oprs_dict = cls.calc_stat(matches, team_list, team_id_map, Minv,
                                  'oprs')
        dprs_dict = cls.calc_stat(matches, team_list, team_id_map, Minv,
                                  'dprs')
        ccwms_dict = cls.calc_stat(matches, team_list, team_id_map, Minv,
                                   'ccwms')
