...
class TeamDetailsDataFetcher(object):
    ...
    def fetch(self, team, year, return_valid_years=False):
        ...
        """
        awards_future = award_query.TeamYearAwardsQuery(team.key.id(),
                                                        year).fetch_async()
        events_future = event_query.TeamYearEventsQuery(team.key.id(),
                                                        year).fetch_async()
        matches_future = match_query.TeamYearMatchesQuery(team.key.id(),
                                                          year).fetch_async()
