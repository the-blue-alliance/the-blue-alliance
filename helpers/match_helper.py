...
class MatchHelper(object):
    ...
    def add_match_times(cls, event, matches):
        ...
        if event.timezone_id is None:  # Can only calculate match times if event timezone is known
            logging.warning(
                'Cannot compute match time for event with no timezone_id: {}'.
        ...
        cur_date = event.end_date + datetime.timedelta(
            hours=23, minutes=59,
            seconds=59)  # end_date is specified at midnight of the last day
        for match in matches_reversed:
            ...
            match_time = datetime.datetime(cur_date.year, cur_date.month,
                                           cur_date.day, hour, minute)
            if last_match_time is not None and last_match_time + datetime.timedelta(
                    hours=6) < match_time:
            ...
            for alliance_color in ['red', 'blue']:
                ...
                for team in match.alliances[alliance_color]['teams']:
                    ...
                    if team in surrogate_teams and num_played[team] == 3:
                        match.alliances[alliance_color]['surrogates'].append(
                            team)
    ...
    def organizeDoubleElimMatches(cls, organized_matches):
        ...
        for level in Match.COMP_LEVELS:
            ...
            for match in level_matches:
                bracket = PlayoffType.get_double_elim_bracket(
                ...
                if event.playoff_type != PlayoffType.ROUND_ROBIN_6_TEAM or match.comp_level == 'f':  # Don't delete round robin semifinal matches
                    ...
                    if red_win_counts[key] == n or blue_win_counts[key] == n:
                        try:
                            ...
                            logging.warning(
                                "Deleting invalid match: %s" % match.key_name)
                        except:
                            logging.warning(
                    ...
                    for i, complete_alliance in enumerate(
                        ...
                        if len(
                            ...
                            for team_num in cls.getOrderedAlliance(
                                    ...
                                    alliance_selections):
                                if team_num not in bracket_table[comp_level][
                                        set_number]['{}_alliance'.format(
                                            color)]:
                                    bracket_table[comp_level][set_number][
                                        '{}_alliance'.format(color)].append(
                                            team_num)
    ...
    def generatePlayoffAdvancement2015(cls, matches, alliance_selections=None):
        ...
        advancement = defaultdict(
            list
        )  # key: comp level; value: list of [complete_alliance, [scores], average_score]
        for comp_level in ['ef', 'qf', 'sf']:
            for match in matches.get(comp_level, []):
                ...
                for color in ['red', 'blue']:
                    ...
                    is_new = False
                    if i is not None:
                        for j, (complete_alliance, scores, _) in enumerate(
                            ...
                            if len(
                                ...
                                advancement[comp_level][j][2] = float(
                    ...
                    if i is None:
                        ...
                        advancement[comp_level].append(
                            [complete_alliances[i], [score], score])

            advancement[comp_level] = sorted(
                advancement[comp_level],
                key=lambda x: -x[2])  # sort by descending average score

        return advancement

    @classmethod
    def generatePlayoffAdvancementRoundRobin(cls,
                                             matches,
                                             alliance_selections=None):
        complete_alliances = []
        alliance_names = []
        advancement = defaultdict(
            list
        )  # key: comp level; value: list of [complete_alliance, [champ_points], sum_champ_points, [match_points], sum_match_points
        for comp_level in ['sf']:  # In case this needs to scale to more levels
            ...
            for match in matches.get(comp_level, []):
                ...
                for color in ['red', 'blue']:
                    ...
                    if i is not None:
                        for j, (
                                complete_alliance, champ_points, _,
                                match_points, _, _, record
                        ) in enumerate(
                                advancement[comp_level]
                        ):  # search for alliance. could be more efficient
                            if len(
                                ...
                                if match.has_been_played:
                                    ...
                                    advancement[comp_level][j][2] = sum(
                                        champ_points)
                                    advancement[comp_level][j][4] = sum(
                                        match_points)
                                break
                        else:
                            is_new = True

                    score = match.alliances[color][
                        'score'] if match.has_been_played else 0
                    ...
                    if i is None:
                        advancement[comp_level].append([
                            alliance, [cp], cp, [score], score, alliance_name,
                            record
                        ])
                    elif is_new:
                        advancement[comp_level].append([
                            complete_alliances[i], [cp], cp, [score], score,
                            alliance_names[i], record
                        ])

            advancement[comp_level] = sorted(
                advancement[comp_level],
                key=lambda x: -x[4])  # sort by match points
            advancement[comp_level] = sorted(
                advancement[comp_level],
                key=lambda x: -x[2])  # sort by championship points
    ...
    def getOrderedAlliance(cls, team_keys, alliance_selections):
        if alliance_selections:
            for alliance_selection in alliance_selections:  # search for alliance. could be more efficient
                ...
                if len(
                        set(picks).intersection(set(team_keys))
                ) >= 2:  # if >= 2 teams are the same, then the alliance is the same
    ...
    VALID_BREAKDOWNS = {
        2014:
        set(['auto', 'assist', 'truss+catch', 'teleop_goal+foul']),
        2015:
        set([
            'coopertition_points', 'auto_points', 'container_points',
            'tote_points', 'litter_points', 'foul_points'
        ]),
        2016:
        set([
            'adjustPoints', 'autoBoulderPoints', 'autoBouldersHigh',
            'autoBouldersLow', 'autoCrossingPoints', 'autoPoints',
            'autoReachPoints', 'breachPoints', 'capturePoints', 'foulCount',
            'foulPoints', 'position1crossings', 'position2',
            'position2crossings', 'position3', 'position3crossings',
            'position4', 'position4crossings', 'position5',
            'position5crossings', 'robot1Auto', 'robot2Auto', 'robot3Auto',
            'techFoulCount', 'teleopBoulderPoints', 'teleopBouldersHigh',
            'teleopBouldersLow', 'teleopChallengePoints',
            'teleopCrossingPoints', 'teleopDefensesBreached', 'teleopPoints',
            'teleopScalePoints', 'teleopTowerCaptured', 'totalPoints',
            'towerEndStrength', 'towerFaceA', 'towerFaceB', 'towerFaceC'
        ]),
        2017:
        set([
            'adjustPoints', 'autoFuelHigh', 'autoFuelLow', 'autoFuelPoints',
            'autoMobilityPoints', 'autoPoints', 'autoRotorPoints', 'foulCount',
            'foulPoints', 'kPaBonusPoints', 'kPaRankingPointAchieved',
            'robot1Auto', 'robot2Auto', 'robot3Auto', 'rotor1Auto',
            'rotor1Engaged', 'rotor2Auto', 'rotor2Engaged', 'rotor3Engaged',
            'rotor4Engaged', 'rotorBonusPoints', 'rotorRankingPointAchieved',
            'techFoulCount', 'teleopFuelHigh', 'teleopFuelLow',
            'teleopFuelPoints', 'teleopPoints', 'teleopRotorPoints',
            'teleopTakeoffPoints', 'totalPoints', 'touchpadFar',
    ...
    def tiebreak_winner(cls, match):
        ...
        if match.year == 2016:
            ...
            if 'foulPoints' in red_breakdown and 'foulPoints' in blue_breakdown:
                tiebreakers.append((red_breakdown['foulPoints'],
                                    blue_breakdown['foulPoints']))
            else:
            ...
            if 'autoPoints' in red_breakdown and 'autoPoints' in blue_breakdown:
                tiebreakers.append((red_breakdown['autoPoints'],
            ...
            if 'teleopTakeoffPoints' in red_breakdown and 'teleopTakeoffPoints' in blue_breakdown:
                tiebreakers.append((red_breakdown['teleopTakeoffPoints'],
