...
class DistrictTeamManipulator(ManipulatorBase):
    ...
    def getCacheKeysAndControllers(cls, affected_refs):
        return CacheClearer.get_districtteam_cache_keys_and_controllers(
            affected_refs)
    ...
    def postUpdateHook(cls, district_teams, updated_attr_list, is_new_list):
        ...
        """
        for (district_team, updated_attrs) in zip(district_teams,
                                                  updated_attr_list):
            # Enqueue task to calculate district rankings
            try:
                taskqueue.add(
                    url='/tasks/math/do/district_rankings_calc/{}'.format(
                        district_team.district_key.id()),
                    method='GET')
            except Exception:
                logging.error(
                    "Error enqueuing district_rankings_calc for {}".format(
                        district_team.district_key.id()))
    ...
    def updateMerge(self,
        ...
        for attr in attrs:
            if getattr(new_district_team, attr) is not None:
                if getattr(new_district_team, attr) != getattr(
                        old_district_team, attr):
                    setattr(old_district_team, attr,
                            getattr(new_district_team, attr))
