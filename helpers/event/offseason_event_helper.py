...
class OffseasonEventHelper(object):
    ...
    def is_direct_match(cls, tba_event, first_event):
        return tba_event.key_name == first_event.key_name or \
               (tba_event.first_code and tba_event.first_code == first_event.key_name)
    ...
    def categorize_offseasons(cls, year, first_events):
        ...
        year_offseasons = [e for e in year_events if e.event_type_enum == EventType.OFFSEASON]

        matched_events = []
        likely_matched_events = []
        new_events = []

        logging.info("Categorizing {} events from FIRST: {}".format(len(first_events), first_events))
        for event in first_events:
            logging.info("Event: {}".format(event.key_name))
            ...
            if likely_match:
                likely_matched_events.append(event)
                continue
