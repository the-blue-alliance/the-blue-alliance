...
class BlueZoneHelper(object):
    ...
    def get_upcoming_match_predictions(cls, live_events):
        ...
        for event in live_events:
            if event.details and event.details.predictions:
                try:
                    predictions.update(
                        event.details.predictions.get('match_predictions', {}))
                except Exception, e:
    ...
    def update_bluezone(cls, live_events):
        ...
        logging.info(
            ...
            format(current_match.has_been_played
                   if current_match else None, now, cutoff_time))
        to_log += "[BLUEZONE] Current match played? {}, now = {}, cutoff = {}\n".format(
            current_match.has_been_played
            if current_match else None, now, cutoff_time)
        ...
        if bluezone_matches:
            ...
            if bluezone_match.key_name != current_match_key:
                ...
                OutgoingNotificationHelper.send_slack_alert(
                    ...
                    format(now,
                           bluezone_match.key.id(), bluezone_match.time,
